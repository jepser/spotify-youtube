    ...
    <div v-bind:class="{ 'is-visible': searched, 'youtube__create': true }">
      <div class="create">
        ...
        <form v-if="create" v-on:submit.prevent="submitPlaylist">
          ...
          <input type="text" class="create__name" v-model="exported['name']" id="playlist-name">
          <p>{{ songs.length }} {{ songs.length == 1 ? 'song' : 'songs'  }} will be exported.</p>
          <button class="create__submit">Let's do this!</button>
        </form>
      </div>
    </div>
  ...
  methods: {
    ...
    submitPlaylist() {
      var request = gapi.client.youtube.playlists.insert({
        ...
        resource: {
          snippet: {
            title: this.exported.name || 'Spotify to Youtube playlist',
      ...
      this.authorizeApp(request, (response) => {
        ...
        if (result) {
          ...
          this.addSongs(this.playlistId, this.v, 0)
        } else {
    ...
    addSongs(playlistId, songs, index) {
      ...
      console.log('addSongs', songs[index], index)
